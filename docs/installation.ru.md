...
В обоих режимах история ветки и привязки ответов сохраняются атомарно в приватном `runtime/state.json` (0600) и переживают рестарт. Срок возврата — 30 дней бездействия (`GUEST_SESSION_TTL=2592000`). Хранятся исходная пара «запрос/ответ» и до 23 последних пар, всего до 120 000 символов, до 30 000 на сообщение. Лимит — 200 недавних веток плюс выполняющиеся задачи и 10 000 привязок сообщений. `GUEST_PENDING_ANCHOR_TTL` больше не используется. Срок касается состояния gateway, а не архивов Hermes.
...
Само по себе успешное завершение tool call ещё не означает завершённую
доставку. Перед финальным ответом harness обязан скопировать или экспортировать
готовый артефакт в `GUEST_HARNESS_MEDIA_DIR`, проверить существование обычного
файла и поместить `MEDIA:` на отдельную строку. Sidecar сначала отправит файл
владельцу в личку, затем встроит возвращённый Telegram `file_id` в rich article.
Файл, оставленный в project/workspace или временном каталоге, намеренно
отклоняется, даже если сам инструмент генерации завершился успешно.

Для задач по аватару `GUEST_PROFILE_PHOTO_ENABLED=1` позволяет sidecar нативно
вызвать `getUserProfilePhotos`, когда точный `user_id` уже есть в авторе reply
или в сущности `text_mention`. В Hermes передаётся крупнейшая доступная
фотография как локальный reference-файл. Обычный `@username` не содержит Bot
API `user_id`, поэтому такой случай маршрутизируется в безопасную операцию
`download_profile_photo` навыка `userbot`; похожее имя не выбирается
автоматически. `GUEST_TOOL_RECOVERY_ENABLED=1` разрешает ровно одну повторную
попытку harness, если явная задача генерации/выгрузки завершилась без реально
существующего разрешённого файла.
...
1. `@бот` без reply начинает новую ветку.
2. Reply на конкретный ответ бота **с новым `@бот`** продолжает именно эту ветку.
3. Новое обращение без reply начинает другую ветку. Reply на старый ответ позволяет вернуться в прежнюю, пока она не истекла.
4. Reply на чужое сообщение с `@бот` начинает новую ветку и добавляет только это сообщение как цитату. Последующее общение владельца с ботом остаётся в ней.
5. Сообщения без явного обращения и сообщения других пользователей игнорируются. Соседние сообщения чата, вложенные reply и контекст других тем не добавляются.

Сессия изолирована по боту, чату, теме и владельцу. Неизвестный или устаревший ответ бота становится только цитатой для новой ветки. Старые неоднозначные привязки предыдущей версии не импортируются; очередь сохраняется. Запросы одной ветки исполняются последовательно.

Runs получает одинаковые `session_id` и `X-Hermes-Session-Key`, а также явную `conversation_history`. Пустое окно задаётся отдельной системной границей, чтобы Hermes не загружал историю неявно.

Отдельный профиль гостевого агента должен отключать автоматическую общую память, иначе `MEMORY.md` и `USER.md` попадут во все ветки. Bootstrap задаёт это автоматически. Для существующего **гостевого** профиля:
...
Файлы памяти при этом не удаляются. Настройки других профилей, включая trainer, менять не требуется. Перезапустите только gateway гостевого профиля после изменения его конфигурации.
...
В Bot API 10.3 обычные файлы также встраиваются нативным `document`-блоком,
Markdown-таблицы отправляются в компактном виде, а явный HTML-блок
`<blockquote expandable>` превращается в сворачиваемую цитату. Обычные цитаты
не сворачиваются автоматически.
